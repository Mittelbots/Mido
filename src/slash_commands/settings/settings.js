...
module.exports.run = async ({ main_interaction, bot }) => {
    ...
    if (response.error) {
        errorhandler({
            err: response.error,
            message: `Error while LogChannelChange UserID: ${main_interaction.user.id}`,
            fatal: false,
        });
        main_interaction
            .reply({
        ...
        errorhandler({
            err: '',
            message: `${response.message} UserID ${main_interaction.user.id} | GuildID: ${main_interaction.guild.id}`,
            fatal: false,
        });
        main_interaction
            .reply({
...
module.exports.data = new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
        subcommand.setName('removelogchannel').setDescription('Remove your Log Channel.')
    )
    .addSubcommand((subcommand) =>
        subcommand
            .setName('permissions')
            ...
            .addBooleanOption((option) =>
                option
                    .setName('viewuserarchiv')
                    .setDescription(
                        'Does your role have the permission to view the personal Archive'
                    )
                    .setRequired(true)
            )
            .addBooleanOption((option) =>
                option
                    .setName('viewguildarchiv')
                    .setDescription('Does your role have the permission to view the Guild Archive')
                    .setRequired(true)
            )
            .addBooleanOption((option) =>
                option
                    .setName('editguildarchiv')
