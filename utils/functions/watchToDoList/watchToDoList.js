...
const { toDoState_Inactive } = require('../../variables/variables');
...
module.exports.watchToDoList = (bot) => {
    setInterval(async () => {
        ...
        await database
            .query(`SELECT * FROM ${config.tables.mido_todo} WHERE state = ?`, ['1'])
            .then((res) => {
                ...
                res.map(async (task) => {
                    ...
                    if (currentTime.getTime() <= deadlineTime.getTime()) {
                        //DeadLine passed
                        ...
                        async function sendMessageToUser() {
                            ...
                            await database
                                .query(
                                    `UPDATE ${config.tables.mido_todo} SET state = ? WHERE id = ?`,
                                    [toDoState_Inactive, taskid]
                                )
                                .catch((err) => {
        ...
        errorhandler({
            err: `${count} tasks successfully passed the deadline & ${dmCount} DM's successfully sent. [I coudn't sent a DM to ${failedCount} users.]`,
            fatal: false,
        });
    }, 10000); // 10 MIN |  600000
};
