...
module.exports.addProject = async (main_interaction) => {
    ...
    if (!hasPerms) {
        return main_interaction.message
            .reply(lang.errors.noperms)
            .then(async (msg) => {
                await delay(2000);
    ...
    var giveNameMessage = await main_interaction.message.channel
        .send('Bitte gebe einen Namen ein!')
        .catch((err) => {});
    ...
    messageCollector.on('collect', async (reply) => {
        ...
        return await database
            .query(
                `INSERT INTO ${config.tables.mido_projects} (name, color, guild_id) VALUES (?, ?, ?)`,
                [reply.content, randomColor(), reply.guildId]
            )
            .then(async () => {
                errorhandler({
                    err: '',
                    message: `Project added UserID ${main_interaction.user.id} | GuildID: ${main_interaction.guild.id}`,
                    fatal: false,
                });
                createLog({
                    type: 4,
            ...
            .catch((err) => {
                return errorhandler({
                    err,
                    message: lang.projects.errors.failed_add,
    ...
    messageCollector.on('end', async (collected, reason) => {
        ...
        if (reason === 'time') {
            try {
                ...
                main_interaction.message.channel
                    .send({
