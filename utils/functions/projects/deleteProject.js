...
const { addSelectMenu, addConfirmMenu } = require('../toDoList/addSelectMenu');
    ...
    if (!isDelete) {
        ...
        if (main_interaction.values[0].split(' ')[0] === cancel_delete_project) {
            var newEmbedBuilder = new EmbedBuilder()
                .setTitle(
                    categories
                        ? lang.projects.choose_new_project
                        : lang.projects.first_add_new_project
                )
                .setTimestamp();
        ...
        const confirmMessage = await main_interaction.message.channel.send({
            content: `Willst du das Projekt ${
                main_interaction.values[0].split(' ')[0]
            } wirklich löschen?`,
            components: [
                new ActionRowBuilder({
        ...
        confirmCollector.on('collect', async (confirm_interaction) => {
            if (confirm_interaction.values[0] === 'no') {
                confirm_interaction.message.channel
                    .send(`${lang.errors.canceled}`)
                    .then(async (msg) => {
                        await delay(2000);
                ...
                const id = main_interaction.values[0]
                    .split(' ')[0]
                    .slice(4, main_interaction.values[0].split(' ')[0].length);
                const name = main_interaction.values[0].split(' ')[0].slice(0, 3);
                return await database
                    .query(
                        `DELETE FROM ${config.tables.mido_projects} WHERE id = ?; UPDATE mido_todo SET state = ? WHERE cat_id = ?;`,
                        [Number(id), toDoState_Deleted, Number(id)]
                    )
                    .then(async () => {
                        errorhandler({
                            err: '',
                            message: `Project removed UserID ${main_interaction.user.id} | GuildID: ${main_interaction.guild.id}`,
                            fatal: false,
                        });
                        createLog({
                            type: 5,
                    ...
                    .catch((err) => {
                        return errorhandler({
                            err,
                            message: lang.projects.errors.error_at_delete,
                            channel: main_interaction.message.channel,
                        });
                    });
            }
        });
