    ...
    messageCollectorDeleteToDo.on('collect', async (reply) => {
        ...
        if (isNaN(reply.content)) {
            ...
            const task = await database
                .query(`SELECT * FROM ${config.tables.mido_todo} WHERE id = ?`, [reply.content])
                .then((res) => {
                    return res[0];
                })
                .catch((err) => {
            ...
            if (task) {
                return await database
                    ...
                    .then(async () => {
                        errorhandler({
                            err: '',
                            message: `Todo archived UserID ${main_interaction.user.id} | GuildID: ${main_interaction.guild.id}`,
                            fatal: false,
                        });
                        createLog({
                            type: 2,
