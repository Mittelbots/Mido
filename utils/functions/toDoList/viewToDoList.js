...
module.exports.viewToDoList = async (projects, todo, main_interaction) => {
    ...
    await projects.map(async (cat) => {
        let isInProject;
        try {
            isInProject = main_interaction.values[0]
                .split(' ')[0]
                .indexOf(select_ProjectId + cat.id);
        } catch (err) {
            isInProject =
                main_interaction.customId.search(cat.id) ??
                main_interaction.customId.search(config.buttons.add_ToDo.customId);
        }
        if (isInProject !== -1) {
                ...
                await todo.map((todo, index) => {
                    if (index >= start && count <= start + 10) {
                        if (todo.cat_id === cat.id) {
                            newEmbedBuilder.setColor(cat.color);
                            let emoji =
                                todo.state === toDoState_Inactive
                                    ? '🛑'
                                    : todo.state === toDoState_Ready
                                    ? '✅'
                                    : '⏹️';
                            newEmbedBuilder.addFields([
                                {
                                    name: `‎\n${emoji} ${todo.title} ||ID: ${todo.id}||`,
