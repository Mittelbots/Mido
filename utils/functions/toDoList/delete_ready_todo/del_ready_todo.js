...
module.exports.del_ready_todo = async (main_interaction) => {
    ...
    const { error, message, data } = await getToDoByProjectId(
        main_interaction.guild.id,
        project_id
    );
        ...
        if (error)
            return main_interaction.channel
                .send({
                    content:
                        '> ❌ ' +
                        lang.deltodo.errors.failed_to_update +
                        ' ' +
                        lang.errors.try_again_or_contact_support,
                })
                .then(async (msg) => {
                    await delay(3000);
                    msg.delete().catch((err) => {});
    ...
    return await main_interaction.channel
        .send({
