...
module.exports.ProjectInteraction = async (main_interaction) => {
    ...
    if (main_interaction.isSelectMenu() && main_interaction.customId === select_ProjectId) {
        if (main_interaction.values[0].split(' ')[0].indexOf(add_ProjectId) !== -1) {
            //? Menu zum hinzufügen der Projekte
            return await addProject(main_interaction);
        } else if (main_interaction.values[0].split(' ')[0].indexOf(delete_Project) !== -1) {
            //? Menu zum löschen der Projekte
        ...
        main_interaction.isSelectMenu() &&
        main_interaction.customId.split(' ')[0] === delete_ready_todo
    ) {
