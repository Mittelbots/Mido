...
module.exports.buttonInteraction = async ({ main_interaction, projects, todo }) => {
    ...
    if (
        main_interaction.customId.split(' ')[0].indexOf(config.buttons.set_todo_ready.customId) !==
        -1
    ) {
    ...
    if (
        main_interaction.customId.split(' ')[0].indexOf(config.buttons.delete_toDo.customId) !== -1
    ) {
    ...
    if (
        main_interaction.customId.split(' ')[0].indexOf(config.buttons.change_prod.customId) !== -1
    ) {
